...
if (!isNil "isRallyPointPlaced" && {isRallyPointPlaced}) then {
    ...
    if (_remainingTravels > 0) then {
        ...
        [0, round (_finalCost/1.3), 0] remoteExec ["A3A_fnc_resourcesFIA",2];
    };
    ...
    if (_resourcesFIA < _finalCost) exitWith {
        ["Rally Point", format ["Not enough commnand points to place rally point, need %1 to proceed.", str _finalCost]] call SCRT_fnc_misc_showDeniedActionHint;
    };
