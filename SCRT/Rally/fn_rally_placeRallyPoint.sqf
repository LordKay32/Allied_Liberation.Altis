...
private _rootHeight = (_rootPos select 2) - 0.205;
rallyPointRoot setPos [_rootPos select 0, _rootPos select 1, _rootHeight];
rallyPointRoot setDir ((getDir rallyPointRoot) + 180);

private _backpack1 = ["B_Carryall_cbr", _radioWorldPosition] call BIS_fnc_createSimpleObject;
private _backpack1Pos = position _backpack1;
_backpack1 setPos [(_backpack1Pos select 0) - 0.15, (_backpack1Pos select 1) + 0.1, (_rootHeight - 0.12)];
_backpack1 setDir 140;
[_backpack1, -30, 0] call BIS_fnc_setPitchBank;

private _backpack2 = ["B_Carryall_cbr", _radioWorldPosition] call BIS_fnc_createSimpleObject;
private _backpack2Pos = position _backpack2;
_backpack2 setPos [(_backpack2Pos select 0) + 0.15, (_backpack2Pos select 1) + 0.1, (_rootHeight - 0.12)];
_backpack2 setDir 220;
[_backpack2, -30, 0] call BIS_fnc_setPitchBank;

private _bag = ["Land_Sleeping_bag_brown_folded_F", _radioWorldPosition] call BIS_fnc_createSimpleObject;
private _bagPos = position _bag;
_bag setPos [(_bagPos select 0) + 0.025, (_bagPos select 1) - 0.1, (_bagPos select 2) + 0.3];
...
rallyPointMarker setMarkerType "mil_end";
