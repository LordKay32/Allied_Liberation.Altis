...
private _posRight = [_staticPosition, 3, (_staticDirection + 90)] call BIS_Fnc_relPos;
{
    private _relativePosition = [_posRight, 4.5,(_staticDirection + _x)] call BIS_Fnc_relPos;
...
private _groupATUnits = _groupXUnits;

if (count _statics > 0) then {
	private _veh = objNull;
	_veh = createVehicle [staticATteamPlayer, _posRight, [], 0, "CAN_COLLIDE"];
	_veh setDir _staticDirection;
	_veh addEventHandler ["Killed", {
			_markerX = [atpostsFIA, _unit] call BIS_fnc_nearestPosition;
	...
	private _crewManIndex = _groupATUnits findIf  {(_x getVariable "unitType") == UKMil};
	if (_crewManIndex != -1) then {
	    private _crewMan = _groupATUnits select _crewManIndex;
	    _crewMan moveInGunner _veh;
		_crewMan doWatch (_veh getRelPos [200, 0]);
	    _groupATUnits deleteAT _crewManIndex;
	};
	_crewManIndex = _groupATUnits findIf  {(_x getVariable "unitType") == UKMil};
	if (_crewManIndex != -1) then {
	    _crewMan = _groupATUnits select _crewManIndex;
...
private _posLeft = [_staticPosition, 3, (_staticDirection + 270)] call BIS_Fnc_relPos;
{
    private _relativePosition = [_posLeft, 4.5, (_staticDirection + _x)] call BIS_Fnc_relPos;
    private _sandbag = createVehicle ["Land_BagFence_Round_F", _relativePosition, [], 0, "CAN_COLLIDE"];
...
{
    _relativePosition = [_staticPosition, 3, (_staticDirection + _x)] call BIS_Fnc_relPos;
    _sandbag = createVehicle ["Land_BagFence_Short_F", _relativePosition, [], 0, "CAN_COLLIDE"];
    _sandbag setDir _staticDirection;
    _sandbag setVectorUp surfaceNormal position _sandbag;
    _props pushBack _sandbag;
} forEach [0, 180];
...
if (count _statics > 1) then {
	private _veh = objNull;
	_veh = createVehicle [staticATteamPlayer, _posLeft, [], 0, "CAN_COLLIDE"];
	_veh setDir _staticDirection;
	_veh addEventHandler ["Killed", {
			_markerX = [atpostsFIA, _unit] call BIS_fnc_nearestPosition;
			_statics = garrison getVariable [(_markerX + "_statics"), []];
