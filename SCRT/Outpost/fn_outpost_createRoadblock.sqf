...
private _textX = format ["%1 Heavy Roadblock", nameTeamPlayer];
private _tsk = "";
...
[[teamPlayer,civilian],_taskId,["We are sending a team to establish a heavy roadblock. Use HC to send the team to their destination.","Roadblock Deploy",_marker],_position,false,0,true,"Move",true] call BIS_fnc_taskCreate;
[_taskId, "outpostTask", "CREATED"] remoteExecCall ["A3A_fnc_taskUpdate", 2];

_formatX = [UKSL,UKMG,UKATman,UKMedic,UKMil,UKMil,UKMil,UKMG];

[-_hrCost,-_moneyCost,_formatX] remoteExec ["A3A_fnc_resourcesFIA",2];

private _count = server getVariable ((_typeVehX select 0) + "_count");
_count = _count - 1;
server setVariable [((_typeVehX select 0) + "_count"), _count, true];

private _count = server getVariable ((_typeVehX select 1) + "_count");
_count = _count - 1;
server setVariable [((_typeVehX select 1) + "_count"), _count, true];
