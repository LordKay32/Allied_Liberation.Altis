params ["_nearX", "_positionTel", "_dirVeh", "_position"];

private _moneyCost = outpostCost select 0;
private _hrCost = outpostCost select 1;

private _textX = format ["%1 Recon Post", nameTeamPlayer];
private _tsk = "";
...
[[teamPlayer,civilian],_taskId,["We are sending a recon team to establish a watchpost. Use HC to send the team to their destination.","Watchpost Deploy",_marker],_position,false,0,true,"Move",true] call BIS_fnc_taskCreate;
[_taskId, "outpostTask", "CREATED"] remoteExecCall ["A3A_fnc_taskUpdate", 2];

[-_hrCost,-_moneyCost,groupsSASRecon] remoteExec ["A3A_fnc_resourcesFIA",2];

_groupX = [getMarkerPos _nearX, teamPlayer, groupsSASRecon] call A3A_fnc_spawnGroup;
