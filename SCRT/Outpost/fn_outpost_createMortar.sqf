...
private _textX = format ["%1 Artillery Emplacement", nameTeamPlayer];
...
[[teamPlayer,civilian],_taskId,["We are sending a team to establish an artillery emplacement. Use HC to send the team to their destination.","Artillery Emplacement Deploy",_marker],_position,false,0,true,"Move",true] call BIS_fnc_taskCreate;
[_taskId, "outpostTask", "CREATED"] remoteExecCall ["A3A_fnc_taskUpdate", 2];

_formatX = [USSL,USMG,USMedic,USATman,USMil,USMil,USMil];

[-_hrCost,-_moneyCost,_formatX] remoteExec ["A3A_fnc_resourcesFIA",2];

if (_typeVehX == "none") then {} else {
