...
if (supportType == "STATIC_MG_AIRDROP" && {_resourcesFIA < 800}) exitWith {
    [
		"FAIL",
		"Support",  
		parseText format ["HQ needs to have at least 800%1 to make this support request.", currencySymbol], 
		30
	] spawn SCRT_fnc_ui_showMessage;
};

if (supportType == "STATIC_MG_AIRDROP" && {(server getVariable (UKMGStatic + "_count")) < 1}) exitWith {
    [
		"FAIL",
		"Support",  
		parseText format ["You need to have at least one Vickers machine gun available.", currencySymbol], 
		30
	] spawn SCRT_fnc_ui_showMessage;
};

if (supportType == "VEH_AIRDROP" && {(server getVariable (vehSDKLightUnarmed + "_count")) < 1}) exitWith {
    [
		"FAIL",
		"Support",  
		parseText format ["You need to have at least one Willy's Jeep available.", currencySymbol], 
		30
	] spawn SCRT_fnc_ui_showMessage;
};
...
if (supportType in ["VEH_AIRDROP", "LOOTCRATE_AIRDROP"] && {_resourcesFIA < 400}) exitWith {
    [
		"FAIL",
		"Support",  
		parseText format ["HQ needs to have at 400%1 to make this support request.", currencySymbol], 
		30
	] spawn SCRT_fnc_ui_showMessage;
};
...
switch (supportType) do {
    ...
    case ("VEH_AIRDROP"): {
        supportPoints = supportPoints - 1;
        publicVariable "supportPoints";
        [0,-400,0] remoteExec ["A3A_fnc_resourcesFIA",2];
    };
    case ("LOOTCRATE_AIRDROP"): {
        supportPoints = supportPoints - 1;
        publicVariable "supportPoints";
        [0,-200,0] remoteExec ["A3A_fnc_resourcesFIA",2];
    };
    case ("STATIC_MG_AIRDROP"): {
        supportPoints = supportPoints - 1;
        publicVariable "supportPoints";
        [0,-800,0] remoteExec ["A3A_fnc_resourcesFIA",2];
    };
    case ("PARADROP"): {
        supportPoints = supportPoints - 1;
        publicVariable "supportPoints";
