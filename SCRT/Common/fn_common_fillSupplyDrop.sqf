...
private _unlockedMagazines = (jna_dataList select IDC_RSCDISPLAYARSENAL_TAB_CARGOMAGALL) select {_x select 1 > 20};
private _unlockedThrowables = (jna_dataList select IDC_RSCDISPLAYARSENAL_TAB_CARGOTHROW) select {_x select 1 > 20};
...
_box addItemCargoGlobal ["fow_i_fak_uk", _firstAidCount];

_firstAidCount = round random [6,10,16];
_box addItemCargoGlobal ["fow_i_fak_us", _firstAidCount];
