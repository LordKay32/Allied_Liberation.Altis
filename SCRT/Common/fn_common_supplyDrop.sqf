...
if (alive _planeVehicle) then {
    ...
    switch(_supportType) do {
        case ("STATIC_MG_AIRDROP"): {
            ...
            _supplyDrop addBackpackCargoGlobal [UKMGStaticWeap, 1];
            _supplyDrop addBackpackCargoGlobal [UKMGStaticSupp, 1];
            _paraPos = [0, 0, -0.2];
        };
        case ("VEH_AIRDROP"): {
            _paraPos = [0, 0, -0.1];
    ...
    [_supplyDrop, _para] spawn {
        ...
        _smokeShell = (selectRandom _smokeShellVariants) createVehicle (_obj getPos [3, random 360]);

        if(sunOrMoon < 1) then {
            private _chemlightVariants = ["Chemlight_green", "Chemlight_red", "Chemlight_yellow", "Chemlight_blue"];
            _chemLight = (selectRandom _chemlightVariants) createVehicle (_obj getPos [3, random 360]);
        };
