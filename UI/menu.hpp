...
class aiManagement: SimpleMenuBig 
{
	idd = -1;
...
class moneyTransferQuery: SimpleMenuSmall
{
	idd=100;
	...
	class Controls
	{
		...
		{
			idc = 104;
			text = Give 500CP to player;
			x = 0.257187 * safezoneW + safezoneX;
			y = 0.304 * safezoneH + safezoneY;
			colorBackground[] = {"(profilenamespace getvariable ['GUI_BCG_RGB_R',0.376])","(profilenamespace getvariable ['GUI_BCG_RGB_G',0.125])","(profilenamespace getvariable ['GUI_BCG_RGB_B',0.043])",0.9};
			action = "moneyTransferQuery = true;";
		};
		...
		class noButton: SimpleButton
		{
			...
			x = 0.477 * safezoneW + safezoneX;
			y = 0.304 * safezoneH + safezoneY;
			action = "closeDialog 0; moneyTransferQuery = false;";
		};
	};
};
...
class squadActions: SimpleMenuBigger 
{
	...
	class Controls
	{
		...
		class supportTaskButton: SimpleButton
		{
			...
			action = "closeDialog 0; [] spawn A3A_fnc_supportActions";
		};
		...
		class artilleryButton: SimpleButton
		{
			idc = -1;
			...
			action = "closeDialog 0; if (player == theBoss) then {[] spawn A3A_fnc_artySupport} else {[""Artillery Support"", ""Only Player Commander has access to this function""] call A3A_fnc_customHint;};";
		};
		
		class stopButton: SimpleButton
		{
			idc = -1;
...
class infVehActions: SimpleMenuSmall
{
	idd = -1;
