...
private _airportsX = (airportsX + milbases) select {(sidesX getVariable [_x,sideUnknown] != teamPlayer) and (spawner getVariable _x != 0)};
...
while {_totalReinf > 0} do
{
	...
	private _targIndex = _reinfTargets findIf {
		(getMarkerPos (_x#1) distance2d getMarkerPos _airport < distanceForLandAttack)
...

{
    ...
    garrison setVariable [format ["%1_recruit", _x], 36, true];
} forEach airportsX;
