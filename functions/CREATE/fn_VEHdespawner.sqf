...
while {alive _veh} do
{
	...
	private _despawn = call {
		...
		if ((["Synd_HQ"] + (airportsX + milbases + outposts + seaports + factories + resourcesX) select {sidesX getVariable [_x,sideUnknown] == teamPlayer}) findIf {_veh inArea _x} != -1) exitWith {false};
