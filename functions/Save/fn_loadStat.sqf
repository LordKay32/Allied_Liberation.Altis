...
private _specialVarLoads = [
	"watchpostsFIA", "roadblocksFIA", "aapostsFIA", "atpostsFIA", "mortarpostsFIA", "lightroadblocksFIA", "hmgpostsFIA", "supportpostsFIA", "minesX","staticsX","constructionsX","attackCountdownOccupants","antennas","mrkNATO","mrkSDK","prestigeNATO",
	"prestigeCSAT","posHQ","armas","items","backpcks","ammunition","dateX","prestigeOPFOR",
	"prestigeBLUFOR","resourcesFIA","intelPoints","skillFIA", "maxConstructions", "destroyedSites",
...
if (_varName in _specialVarLoads) then {
	...
	if (_varName == 'UKhr') then {server setVariable ["UKhr",_varValue,true]};
	if (_varName == 'SAShr') then {server setVariable ["SAShr",_varValue,true]};
	if (_varName == 'UShr') then {server setVariable ["UShr",_varValue,true]};
	if (_varName == 'parahr') then {server setVariable ["parahr",_varValue,true]};
	if (_varName == 'SDKhr') then {server setVariable ["SDKhr",_varValue,true]};
	...
	if (_varName == 'baseMarkersX') then {
		{
			_x setMarkerAlpha 1;
		} forEach _varValue;
	};
	if (_varName == 'mrkAntennas') then {
		{
			_x setMarkerAlpha 1;
		} forEach _varValue;
	};
	...
	if (_varName == 'intelPoints') then {server setVariable ["intelPoints",_varValue,true]};
		...
		if (count (_varValue select 0) == 2) then {
			{
				...
				[[_mrk],"SCRT_fnc_outpost_createWatchpostDistance"] call A3A_fnc_scheduler;
	...
	if (_varName == 'lightroadblocksFIA') then {
		if (count (_varValue select 0) >= 2) then {
			{
				_positionX = _x select 0;
				_garrison = _x select 1;
				_staticPositions = _x select 2;
				_statics = _x select 3;
				_mrk = createMarker [format ["FIALightRoadblock%1", random 1000], _positionX];
				_mrk setMarkerShape "ICON";
				_mrk setMarkerType "n_support";
				_mrk setMarkerColor colorTeamPlayer;
				_mrk setMarkerText format ["%1 Light Roadblock",nameTeamPlayer];
				spawner setVariable [_mrk,2,true];
				if (count _garrison > 0) then {garrison setVariable [_mrk,_garrison,true]};
				if (count _staticPositions > 0) then {staticPositions setVariable [_mrk,_staticPositions,true]};
				if (count _statics > 0) then {garrison setVariable [_mrk + "_statics",_statics,true]};
				lightroadblocksFIA pushBack _mrk;
				sidesX setVariable [_mrk,teamPlayer,true];
			} forEach _varvalue;
		};
	};
		...
		if (count (_varValue select 0) >= 2) then {
			{
				...
				_mrk setMarkerShape "ICON";
				_mrk setMarkerType "n_art";
				_mrk setMarkerColor colorTeamPlayer;
				_mrk setMarkerText format ["%1 Artillery Emplacement",nameTeamPlayer];
				[[_mrk],"SCRT_fnc_outpost_createMortarDistance"] call A3A_fnc_scheduler;
				if (count _garrison > 0) then {garrison setVariable [_mrk,_garrison,true]};
				if (count _staticPositions > 0) then {staticPositions setVariable [_mrk,_staticPositions,true]};
				if (count _statics > 0) then {garrison setVariable [_mrk + "_statics",_statics,true]};
	...
	if (_varName == 'supportpostsFIA') then {
		if (count (_varValue select 0) >= 2) then {
			{
				_positionX = _x select 0;
				_garrison = _x select 1;
				_staticPositions = _x select 2;
				_statics = _x select 3;
				_mrk = createMarker [format ["FIASupportPost%1", random 1000], _positionX];
				_mrk setMarkerShape "ICON";
				_mrk setMarkerType "n_service";
				_mrk setMarkerColor colorTeamPlayer;
				_mrk setMarkerText format ["%1 Support Post",nameTeamPlayer];
				[[_mrk],"SCRT_fnc_outpost_createSupportPostDistance"] call A3A_fnc_scheduler;
				if (count _garrison > 0) then {garrison setVariable [_mrk,_garrison,true]};
				if (count _staticPositions > 0) then {staticPositions setVariable [_mrk,_staticPositions,true]};
				if (count _statics > 0) then {garrison setVariable [_mrk + "_statics",_statics,true]};
				supportpostsFIA pushBack _mrk;
				sidesX setVariable [_mrk,teamPlayer,true];
			} forEach _varvalue;
		};
	};
	...
	if (_varName == 'posHQ') then {
		...
		if (count _varValue == 3) then {
			...
			flagUK setPos (_varValue select 5);
		};
    ...
    if(_varname == 'finalStatistics') then {
        _finalStatistics = _varvalue;
		occupantKilled = _finalStatistics select 0;
		publicVariable "occupantKilled";
		occupantKilledByPlayers = _finalStatistics select 1;
		publicVariable "occupantKilledByPlayers";
		occupantVehKilled = _finalStatistics select 2;
		publicVariable "occupantVehKilled";
		occupantVehKilledByPlayers = _finalStatistics select 3;
		publicVariable "occupantVehKilledByPlayers";
		playerDeaths = _finalStatistics select 4;
		publicVariable "playerDeaths";
		playerDeathsFF = _finalStatistics select 5;
		publicVariable "playerDeathsFF";
		teamPlayerDeployed = _finalStatistics select 6;
		publicVariable "teamPlayerDeployed";
		teamPlayerStoodDown = _finalStatistics select 7;
		publicVariable "teamPlayerStoodDown";
		teamPlayerKilled = _finalStatistics select 8;
		publicVariable "teamPlayerKilled";
		teamPlayerKilledFF = _finalStatistics select 9; 
		publicVariable "teamPlayerKilledFF";
		partizanKilled = _finalStatistics select 10;
		publicVariable "partizanKilled";
		partizanKilledFF = _finalStatistics select 11;
		publicVariable "partizanKilledFF";
		teamPlayerVehDeployed = _finalStatistics select 12;
		publicVariable "teamPlayerVehDeployed";
		teamPlayerVehDepot = _finalStatistics select 13;
		publicVariable "teamPlayerVehDepot";
		teamPlayerVehKilled = _finalStatistics select 14;
		publicVariable "teamPlayerVehKilled";
		teamPlayerVehKilledFF = _finalStatistics select 15; 
		publicVariable "teamPlayerVehKilledFF";
		civilianKilledByOccupant = _finalStatistics select 16;
		publicVariable "civilianKilledByOccupant";
		civilianKilledByteamPlayer = _finalStatistics select 17;
		publicVariable "civilianKilledByteamPlayer";
		prisonersCaptured = _finalStatistics select 18;
		publicVariable "prisonersCaptured";
		vehiclesCaptured = _finalStatistics select 19;
		publicVariable "vehiclesCaptured";
		sectorsLiberated = _finalStatistics select 20;
		publicVariable "sectorsLiberated";
		sectorsLost = _finalStatistics select 21;
