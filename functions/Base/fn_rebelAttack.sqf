...
private _findIfNearAndHostile = {
	...
	*/
	...
	{
		_friendlyBase = _x;
		_MarkerList = _markers select {(getMarkerPos _x distance2D getMarkerPos _friendlyBase < distanceMission)};
		{
...
_possibleTargets = [_possibleTargets] call _findIfNearAndHostile;
...
private _availableTargets = [];
{
    ...
    private _startAirportPos = getMarkerPos _startAirport;
    {
        ...
        if(_distance < distanceForAirAttack) then
        {
            ...
            private _nearbyFriendlyMarkers = (markersX - controlsX - citiesX - watchpostsFIA - roadblocksFIA - aapostsFIA - atpostsFIA - mortarpostsFIA - lightroadblocksFIA - hmgpostsFIA - supportpostsFIA) select
            {
