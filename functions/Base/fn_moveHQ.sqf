...
if (count _garrison > 0) then
{
        ...
        private _size = ["Synd_HQ"] call A3A_fnc_sizeMarker;
        {
            if ((side (group _x) == teamPlayer) && (!(_x getVariable ["spawner",false])) && (_x distance2D _posHQ < _size) && (_x != petros)) then
            {
                if (!alive _x) then
                {
                    ...
                    {
                        if (_unitType in [USstaticCrewTeamPlayer,UKstaticCrewTeamPlayer]) then
                        {
    ...
    ["Garrison", format ["Garrison removed<br/><br/>Recovered CP: %1 %3<br/>Recovered HR: %2",_costs,_hr, currencySymbol]] call A3A_fnc_customHint;
};
