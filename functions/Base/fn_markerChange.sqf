...
if ((!(_markerX in citiesX)) and (spawner getVariable _markerX != 2)) then
	...
	_flagIndex = _flagsX findIf {flagTexture _x == "a3\data_f\flags\flag_us_co.paa"};
	_UKIndex = if (_flagIndex == 0) then {1} else {0};
	
	_flagX = _flagsX select _flagIndex;
	_UKFlag = _flagsX select _UKIndex;
	};
...
if (_winner == teamPlayer) then
{
	...
	if (!isNull _flagX) then
	{
		...
		if (_markerX in milbases) then {[_flagX,"SDKFlag"] remoteExec ["A3A_fnc_flagaction",[teamPlayer,civilian],_flagX]};
