...
private _possibleBases = (outposts + airportsX + milbases) select
{
    ...
    {((getMarkerPos _x) distance2D _supportPos <= 1950) &&
    {spawner getVariable [_x, -1] == 2}}
};

if (count _possibleBases == 0) then {_isMortar = false};
...
else
{
	...
	private _possibleBases = _artyMarkers select
    {
        (sidesX getVariable [_x, sideUnknown] == _side) &&
        {((getMarkerPos _x) distance2D _supportPos <= 9000) &&
        {((getMarkerPos _X) distance2D _supportPos > 2000) &&
        {spawner getVariable [_x, -1] == 2}}} &&
        (count (nearestObjects [(getMarkerPos _x), [NATOHowitzer], 250]) > 0)
    };
...
//Spawn in crew
_mortarGroup = [_side, _mortar] call A3A_fnc_createVehicleCrew;
...
private _sleepTime = 20; //_minSleepTime + random (_setupTime - _minSleepTime);
