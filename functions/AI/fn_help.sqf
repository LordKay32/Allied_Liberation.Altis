...
if (_medicX != _unit) then {
	if !(_unit getVariable ["incapacitated",false]) then {
		if (_isPlayer) then {_unit groupChat format ["This is %1. I'm hurt!",name _unit]};
	...
	[_medicX,_unit,_enemy] spawn {
		params ["_medicX","_unit","_enemy"];
		waitUntil {sleep 1; (_medicX distance _unit < 20) || !([_medicX] call A3A_fnc_canFight)};
		if (_medicX distance _unit < 20) then {[_medicX,_unit,_enemy] call A3A_fnc_chargeWithSmoke};
	};
