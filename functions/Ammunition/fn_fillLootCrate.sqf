...
//Loot contents adjusted by J.Bennett for WW2.
//Weapons
private _lootRifles = ["LIB_K98","LIB_G43"];
private _lootHandguns = ["LIB_M1908"];	
private _lootMachineGuns = ["LIB_MG34","LIB_MG42","LIB_FG42G"];
private _lootSMGs = ["LIB_MP40"];
private _lootSniperRifles = ["LIB_K98ZF39"];
private _lootRocketLaunchers = ["LIB_RPzB","LIB_PzFaust_30m","LIB_PzFaust_60m"];
//Items
private _lootItems = ["fow_i_fak_ger","Medikit","ToolKit","U_LIB_GER_Schutze","U_LIB_GER_Unterofficer","U_LIB_GER_Medic","U_LIB_GER_Oberschutze","U_LIB_GER_MG_schutze", "U_LIB_GER_Soldier_camo2", "U_LIB_GER_MG_schutze", "U_LIB_GER_Soldier_camo2"];
//Magazines
private _lootMagazines = ["LIB_5Rnd_792x57","LIB_10Rnd_792x57","LIB_20Rnd_792x57","LIB_32Rnd_9x19","LIB_50Rnd_792x57","LIB_8Rnd_9x19_P08","LIB_1Rnd_RPzB", "LIB_1Rnd_G_PZGR_30", "LIB_1Rnd_G_PZGR_40", "LIB_1Rnd_G_SPRGR_30","LIB_Shg24", "LIB_M39"];
//Explosives
private _lootExplosives = ["LIB_TMI_42_MINE_mag","LIB_shumine_42_MINE_mag","LIB_SMI_35_MINE_mag","LIB_Ladung_Small_MINE_mag", "LIB_Ladung_Big_MINE_mag"];
//Attachments
private _lootAttachments = ["LIB_ACC_GW_SB_Empty","LIB_Optic_Zf4"];
//Backpacks
private _lootBackpacks = ["B_LIB_GER_SapperBackpack_empty","B_LIB_GER_MedicBackpack_empty", "B_LIB_GER_Panzer_Empty"];
//Helmets
private _lootHelmets = ["H_LIB_GER_Helmet", "H_LIB_GER_Cap", "H_LIB_GER_Helmet", "H_LIB_GER_Helmet", "H_LIB_GER_HelmetCamo"];
//Vests
private _lootVests = ["V_LIB_GER_VestKar98","V_LIB_GER_VestUnterofficer","V_LIB_GER_VestMG","V_LIB_GER_PioneerVest","V_LIB_GER_SniperBelt"];
...
private _weaponLootInfo = [
	[_lootRifles, unlockedRifles, 3],
	[_lootHandguns, unlockedHandguns, 1.2],
	[_lootMachineGuns, unlockedMachineGuns, 2],
	[_lootSMGs, unlockedSMGs, 2],
	[_lootSniperRifles, unlockedSniperRifles, 0.9],
	[_lootRocketLaunchers, unlockedRocketLaunchers, 0.5]
];
...
if (_crateItemTypeMax != 0) then {
	...
	for "_i" from 0 to floor random _crateItemTypeMax do {
		_available = (_lootItems - _unlocks - itemCargo _crate);
		[4, format ["Breakdown: %1, %2, %3", _lootItems, _unlocks, itemCargo _crate], _filename] call A3A_fnc_log;
...
if (_crateAmmoTypeMax != 0) then {
	for "_i" from 0 to floor random _crateAmmoTypeMax do {
		_available = (_lootMagazines - _unlocks - itemCargo _crate);
		_loot = selectRandom _available;
		if (isNil "_loot") then {
...
if (_crateExplosiveTypeMax != 0) then {
	for "_i" from 0 to floor random _crateExplosiveTypeMax do {
		_available = (_lootExplosives - _unlocks - itemCargo _crate);
		_loot = selectRandom _available;
		if (isNil "_loot") then {
...
if (_crateAttachmentTypeMax != 0) then {
	for "_i" from 0 to (_crateAttachmentTypeMax call _fnc_pickNumberOfTypes) do {
		_available = (_lootAttachments - _unlocks - itemCargo _crate);
		_loot = selectRandom _available;
		if (isNil "_loot") then {
...
if (_crateBackpackTypeMax != 0) then {
	for "_i" from 0 to floor random _crateBackpackTypeMax do {
		_available = (_lootBackpacks - _unlocks - itemCargo _crate);
		_loot = selectRandom _available;
		if (isNil "_loot") then {
...
if (_crateHelmetTypeMax != 0) then {
	for "_i" from 0 to floor random _crateHelmetTypeMax do {
		_available = (_lootHelmets - _unlocks - itemCargo _crate);
		_loot = selectRandom _available;
		if (isNil "_loot") then {
...
if (_crateVestTypeMax != 0) then {
	for "_i" from 0 to floor random _crateVestTypeMax do {
		_available = (_lootVests - _unlocks - itemCargo _crate);
		_loot = selectRandom _available;
		if (isNil "_loot") then {
