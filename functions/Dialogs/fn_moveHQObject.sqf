...
if (_playerX distance2D _positionX > (_size + 5) && count (nearestObjects [player, [vehSDKAmmo], 100]) == 0) exitWith {["Move HQ", "This asset needs to be closer to it relative zone center to be able to be moved."] call A3A_fnc_customHint;};
...
waitUntil {sleep 1;
	...
	or (!(_playerX inArea _markerX) && (count (nearestObjects [_playerX, [vehSDKAmmo], 100]) == 0))
...
if  (_playerX distance2D _positionX > (_size-3) && count (nearestObjects [player, [vehSDKAmmo], 100]) == 0) exitWith {["Move HQ", "This asset cannot be moved more far away for its zone center."] call A3A_fnc_customHint;};
