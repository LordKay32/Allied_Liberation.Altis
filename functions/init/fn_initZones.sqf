...
(seaMarkers + seaSpawn + seaAttackSpawn + spawnPoints + detectionAreas + mineMarkers) apply {_x setMarkerAlpha 0};
...
_cityConfigs apply {
	...
	if (_nameX in ["Sofia","Paros","Charkia","Pyrgos","Athira","AgiosDionysios","Zaros","Kavala","Neochori"]) then {
				_dmrk setMarkerSize [0.75, 0.75];
				_dmrk setMarkerType "plp_mark_civ_townhall";
			} else {
				_dmrk setMarkerSize [0.6, 0.6];
				_dmrk setMarkerType "plp_mark_as_house";
			};
...
majorCitiesX = ["Sofia","Paros","Charkia","Pyrgos","Athira","AgiosDionysios","Zaros","Kavala","Neochori"];
villagesX = ["AgiaTriada","AgiosPetros","Delfinaki","Ekali","Koroni","Nifi"];
townsX = citiesX - (majorCitiesX + villagesX);
...
"Land_Vysilac_FM","Land_A_TVTower_base","Land_Telek1", "Land_vn_tower_signal_01", "Land_TTowerSmall_1_F", "Land_TTowerBig_2_ruins_F"];
...
private _replaceBadAntenna = {
	params ["_antenna"];
	if ((typeof _antenna) in _antennatypes) then {
		...
		_antennaPos set [2, 0];
		private _antennaClass = "Land_Vysilac_FM2";
...
switch (toLower worldName) do {
	...
	case "altis": {
		_posAntennas =
		[[26725.3,24587,0], [23631.6,21066.5,0], [20944.3,19278.7,0], [16084.7,16998.1,0], [26455.2,22168.5,0], [19352,9691.72,0], [17449.2,13130.9,9.53674e-007], [20840.2,7242.36,0],
		[14451.6,16338.2,0], [9223.61,19250.2,0], [9129.43,21616.7,0], [11630.1,12014.1,0], [8268.21,10051.3,0], [4584.6,15401.1,0], [6841.33,16162.9,0], [3942.39,12269.2,0]];
		_blacklistPos = [4];
		_posBank = [];
		antennas = [];
	};
...
publicVariable "lightroadblocksFIA";
publicVariable "hmgpostsFIA";
publicVariable "supportpostsFIA";
