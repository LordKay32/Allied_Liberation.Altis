...
//	waitUntil {!isNil"statsLoaded"};
	...
	{
	_unit = [_groupX, _x, [0,0,0]] call A3A_fnc_createUnit; 
	[_unit] call A3A_fnc_FIAInit;
	_loadout = getUnitLoadout _unit;
	rebelLoadouts set [_x, _loadout];
	deleteVehicle _unit;
	} forEach (UKTroops + SASTroops + USTroops + paraTroops);
	deleteGroup _groupX;
...
if !(loadLastSave) then {
	...
	//Add weapons + mags + items to box
	{[_x select 0 call jn_fnc_arsenal_itemType, _x select 0, _x select 1] call jn_fnc_arsenal_addItem} forEach (WW2Weapons + WW2Magazines + WW2Items);
