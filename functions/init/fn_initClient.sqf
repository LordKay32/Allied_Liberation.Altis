...
["StartingIntro", false, 5] call BIS_fnc_blackOut;
...
_introShot = if (introFinished) then {
	[
...
if ((isPlayer _killer) && (_killer != player)) then {
		playerDeathsFF = playerDeathsFF + 1;
...
if ((_this select 3 == 'healsoldier') && ((_this select 0) getVariable ['incapacitated',false])) then { 
['Revive', format ['%1 needs reviving first.',name (_this select 0)]] call A3A_fnc_customHint;; 
true 
}"];
