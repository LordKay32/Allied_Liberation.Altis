...
switch (_type) do
{
    case "MILITARY":
    {
		...
		{
		    ["Undercover", "You can only go Undercover as the Werhmacht with SAS units in your group."] call A3A_fnc_customHint;
		...
		{
		    ["Undercover", "Only SAS units can go Undercover as the Werhmacht."] call A3A_fnc_customHint;
		    [false, "No Wehrmacht Undercover with non SAS troops"];
		};

		if !(isNull (objectParent player)) then
		{
		    if (!(typeOf(objectParent player) in (vehNATONormal + vehNATOAPC))) exitWith
		    {
		        ["Undercover", "You are not in a suitable Wehrmacht vehicle."] call A3A_fnc_customHint;
		        _result = [false, "In non Wehrmacht vehicle"];
		    };
		    if ((objectParent player) in reportedVehs) then
		    {
		        ["Undercover", "This vehicle has been reported to the enemy."] call A3A_fnc_customHint;
	...
	case "CIVILIAN":
    {
		...
		{
		    ["Undercover", "Only partizan units can go Undercover."] call A3A_fnc_customHint;
