...
if (
	...
	secondaryWeapon player in (wehrmachtWeapons + [""]) &&
	handgunWeapon player in (wehrmachtWeapons + [""])
	) then {_undercoverType = "MILITARY"} else {"CIVILIAN"};
...
switch (_undercoverType) do
{
    case "MILITARY":
    {
		...
		/*...Non-nazi vehicle
		...Started shooting , threw grenade, planted bomb if seen
		...other same as below as relevant
		...on base too long?
		...too close to German soldier?*/
		...
		player setVariable ["militaryUndercover", true, true];
...
switch (_reason) do
{
    ...
    case "NoPartizans":
    {
        ["Undercover", "You don't have any partizans in your group to aid your cover as Greek civilians."] call A3A_fnc_customHint;
    };
    case "NonPartizans":
    {
    ...
    case "clothes":
    {
        ["Undercover", "You cannot stay Undercover while:<br/><br/>A weapon is visible<br/>Wearing a vest<br/>Wearing a helmet<br/>Wearing a mil uniform!"] call A3A_fnc_customHint;
    };
    case "clothes2":
    {
        ["Undercover", "You cannot stay Undercover while showing:<br/><br/>A weapon is visible<br/>Wearing a vest<br/>Wearing a helmet<br/>Wearing a mil uniform<br/><br/>The enemy added you to their Wanted List!"] call A3A_fnc_customHint;
        player setVariable["compromised", dateToNumber[date select 0, date select 1, date select 2, date select 3, (date select 4) + 30]];
    };
    case "BadMedic":
    {
