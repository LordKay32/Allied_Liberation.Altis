...
if ((units group player) findIf {_x in SASTroops} == -1) exitWith
{
    ["Undercover", "You can only go Undercover as the Werhmacht with SAS units in your group with full Werhmacht gear."] call A3A_fnc_customHint;
    [false, "No Werhmacht Undercover without SAS"];
};

if ((units group player) findIf {_x in (alliedTroops - SASTroops)} != -1) exitWith
{
    ["Undercover", "Only SAS units in Werhmacht gear can go Undercover as the Werhmacht."] call A3A_fnc_customHint;
    [false, "No Wehrmacht Undercover with non SAS troops"];
};

/*if ((units group player) findIf {!(_x getVariable "WehrmachUniform", false)} != -1) exitWith
{
    ["Undercover", "Your SAS units must be in Werhmacht gear to go undercover."] call A3A_fnc_customHint;
    [false, "No Undercover with non partizan troops"];
};*/

//other to code
/*if weapon/vest/uniform/helmet combo not right

if in non Werhmacht vehicle

if cover broken, 30 mins etc.*/
...
if ((units group player) findIf {_x in SDKTroops} == -1) exitWith
{
    ["Undercover", "You can only go Undercover with partizans in your group."] call A3A_fnc_customHint;
    [false, "No Undercover without partizans"];
};

if ((units group player) findIf {_x in (alliedTroops - SDKTroops)} != -1) exitWith
{
    ["Undercover", "Only partizans can go Undercover."] call A3A_fnc_customHint;
    [false, "No Undercover with non partizan troops"];
};
...
if !(isNull (objectParent player)) then
{
    ...
    {
        ["Undercover", "This vehicle has been reported to the enemy. Return the vehicle to the partizans or HQ to go Undercover again."] call A3A_fnc_customHint;
