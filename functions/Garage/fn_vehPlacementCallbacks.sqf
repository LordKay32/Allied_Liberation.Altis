	...
	case "BUYFIA": {
		switch (_callbackType) do {
			...
			case CALLBACK_SHOULD_CANCEL_PLACEMENT: {
				if (!(player inArea vehiclePurchase_nearestMarker) && count (nearestObjects [player, [vehSDKAmmo], 100]) == 0) exitWith {
					[true, "You need to be closer to the flag or ammo truck to be able to deploy a vehicle"];
				};
				[false];
			};
			...
			case CALLBACK_VEH_IS_VALID_LOCATION: {
				...
				private _maxDist = [100,200] select ((_callbackParams select 2) isKindOf "Ship");
				if ((_pos distance2d (getMarkerPos vehiclePurchase_nearestMarker) > _maxDist) && count (nearestObjects [player, [vehSDKAmmo], 100]) == 0) exitWith
				{
		...
		switch (_callbackType) do {
			...
			case CALLBACK_VEH_PLACED_SUCCESSFULLY: {
				...
				if (!isMultiplayer) then {
					...
					if (player == theBoss && {vehiclePurchase_cost <= _factionMoney}) then {
						_nul = [0,(-1 * vehiclePurchase_cost),0] remoteExec ["A3A_fnc_resourcesFIA",2];
					}
					else {
						[-1 * vehiclePurchase_cost] call A3A_fnc_resourcesPlayer;
						_purchasedVeh setVariable ["ownerX",getPlayerUID player,true];
