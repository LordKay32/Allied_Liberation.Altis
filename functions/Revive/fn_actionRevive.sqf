...
{
    if (_player) then {["Revive", format ["%1 is severely injured, only a medic can revive him.",name _cured]] call A3A_fnc_customHint;};
    if (_inPlayerGroup) then {_medic groupChat format ["%1 is severely injured, only a medic can revive him.",name _cured]};
    false
};
...
private _nearEnemy = _medic findNearestEnemy _cured;
private _smoke = (getPos _cured) nearObjects ["SmokeShell", 25];
		
if ((count _smoke == 0) && insideBuilding _cured == 0 && isPlayer _cured && !(isPlayer _medic) && _nearEnemy distance _cured < 250 && magazines _medic findIf {_x in allSmokeGrenades} != -1) then {
	_enemyDir = _cured getDir _nearEnemy;
	_smokePos = _cured getPos [3, _enemyDir];
	_magazines = magazines _medic select {_x in allSmokeGrenades};
	_smokeGrenade = selectRandom _magazines;
	_medic removeItem _smokeGrenade;
	_smokeGrenade createVehicle _smokePos;
	sleep 4;
};		

//
if (!(isPlayer _medic)) then {
	_medic doWatch _cured;
	_medic lookAt _cured;
	sleep 2;
};

private _timer = if (_isMedic) then { time + 30 } else { time + 60 };
