...
[
	...
	_taskId,
	[
		format ["The %3 is rebuilding a radio tower in %1. If we want to keep up the enemy comms breakdown, the work must be stopped. Destroy the repair truck parked nearby or capture the zone. The work will be finished by %2.<br/><br/>Reward: 500CP per player.",_nameDest,_displayTime,nameOccupants],
...
if (spawner getVariable _markerX != 2) then
	...
	if (not alive _veh) then
		...
		{if (_x distance _veh < 500) then {[50,_x] call A3A_fnc_playerScoreAdd}} forEach (allPlayers - (entities "HeadlessClient_F"));
