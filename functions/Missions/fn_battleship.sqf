...
_displayTime = [_dateLimit] call A3A_fnc_dateToTimeString;//Converts the time portion of the date array to a string for clarity in hints
		...
		_reward = if (_difficultX) then {4000} else {2000};
	...
	//spawn convoy
		...
		if (_objectives findIf { alive _x } == -1) then {
			...
			[0,2000*_bonus,0] remoteExec ["A3A_fnc_resourcesFIA",2];
   	        [0,10*_bonus,_positionX] remoteExec ["A3A_fnc_citySupportChange",2];
			[1200*_bonus, _sideX] remoteExec ["A3A_fnc_timingCA",2];
			{ [200*_bonus, _x] call A3A_fnc_playerScoreAdd } forEach (call BIS_fnc_listPlayers) select { side _x == teamPlayer || side _x == civilian};
