...
if (_side == Invaders) then {
	...
	_squad = CSATSquad;
	_difficultX = if (aggressionLevelInvaders > 3) then {true} else {false};
} else {
	_sideName = nameOccupants;
	_bodyGuard = policeOfficer;
...
_traitor setUnitLoadout [ [], [], [],    ["", []], [], [],    "", "", [], ["ItemMap","","","ItemCompass","ItemWatch",""] ];	

_traitor forceAddUniform (selectRandom ["U_GELIB_FRA_MGunner_gvnpFF13","U_GELIB_FRA_MGunner_gvmpFF14","U_GELIB_FRA_SoldierFF_gvmpFF15","U_GELIB_FRA_SoldierFF_gvmpFF16"]);
_traitor addHeadgear (selectRandom ["H_LIB_SOV_RA_PrivateCap_VDVse","GEH_Beret_blue","GERDS_Beret1"]);
...
_veh = selectRandom vehNATOLightArmed createVehicle _posVeh;
...
if (_difficultX) then {
	_apcVehicleType = selectRandom ["fow_v_sdkfz_222_camo_ger_heer", "fow_v_sdkfz_234_1"];
	_center = _veh getRelPos [10, 180];
	_apcpos = _center findEmptyPosition [0, 50, _apcVehicleType];
	_apc = [_apcpos, _dirveh, _apcVehicleType, _side] call A3A_fnc_spawnVehicle;
	_group = _apc select 2;
	{
	[_x] joinSilent _groupTraitor;
	} forEach units _group;
	deleteGroup _group;
	[_apc select 0, _side] call A3A_fnc_AIVEHinit;
};
...
_sol2 assignAsGunner _veh;
_sol2 moveInGunner _veh;
...
waitUntil {sleep 1; (traitorIntel) || {(dateToNumber date > _dateLimitNum) or {(not alive _traitor) or {({_traitor knowsAbout _x > 0.1 && captive _x == false} count ([500,0,_traitor,teamPlayer] call A3A_fnc_distanceUnits) > 0)}}}};

if ({_traitor knowsAbout _x > 0.1 && captive _x == false} count ([500,0,_traitor,teamPlayer] call A3A_fnc_distanceUnits) > 0) then
	{
	{_x enableAI "MOVE"} forEach units _groupTraitor;
	_traitor assignAsCargo _veh;
	[_traitor] orderGetin true;
	_sol1 assignAsDriver _veh;
	[_sol1] orderGetin true;
...
if (not alive _traitor || traitorIntel) then
{
	...
	{
		{[petros,"hint","Someone found some intel on the traitor's family, he will not cause any problems any more!"] remoteExec ["A3A_fnc_commsMP",_x]} forEach ([500,0,_traitor,teamPlayer] call A3A_fnc_distanceUnits);
	...
	_factor = if (_difficultX) then {1.5} else {1};
...
else
{
	...
	if (_difficultX) then {[-20,theBoss] call A3A_fnc_playerScoreAdd} else {[-10,theBoss] call A3A_fnc_playerScoreAdd};
	if (dateToNumber date > _dateLimitNum) then
	{
		_hrT = server getVariable "SDKhr";
		_resourcesFIAT = server getVariable "resourcesFIA";
		[-1*(round(_hrT/3)),-1*(round(_resourcesFIAT/5)),SDKSL] remoteExec ["A3A_fnc_resourcesFIA",2];
